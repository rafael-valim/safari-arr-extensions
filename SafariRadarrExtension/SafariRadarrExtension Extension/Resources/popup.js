...
document.addEventListener('DOMContentLoaded', async () => {
  const hostInput = document.getElementById('radarrHost');
  const apiKeyInput = document.getElementById('radarrApiKey');
  const fetchRadarrButton = document.getElementById('fetchRadarrConfig');
  ...
  let radarrConfigurationLoaded = false;
  ...
  hostInput.value = settings.radarrHost || '';
  apiKeyInput.value = settings.radarrApiKey || '';

  // Store initial values to track changes (start with current form state)
  const initialValues = {
    radarrHost: hostInput.value,
    radarrApiKey: apiKeyInput.value,
  ...
  function checkSaveButtonState() {
    const currentValues = {
      radarrHost: hostInput.value.trim(),
      radarrApiKey: apiKeyInput.value.trim(),
      qualityProfileId: profileSelect.value,
      rootFolderPath: folderSelect.value
    };

    // Check if all required fields are filled
    const allFieldsFilled = currentValues.radarrHost &&
                           currentValues.radarrApiKey &&
  ...
  function checkFetchButtonState() {
    ...
    if (loadedConfig.host && loadedConfig.apiKey && loadedConfig.host === currentHost && loadedConfig.apiKey === currentApiKey) {
      radarrConfigurationLoaded = true;
      fetchRadarrButton.textContent = 'Configuration Loaded';
      fetchRadarrButton.disabled = true;
      return;
    }

    // If configuration was loaded but current values are different, show "Fetch Configuration"
    if (radarrConfigurationLoaded && hasHost && hasApiKey) {
      fetchRadarrButton.textContent = 'Fetch Configuration';
      fetchRadarrButton.disabled = false;
      return;
    }

    // Default logic: enable only if both fields are populated
    const canFetch = hasHost && hasApiKey;
    fetchRadarrButton.disabled = !canFetch;
    fetchRadarrButton.textContent = 'Fetch Configuration';
  }
  ...
  async function checkPermissionStatus() {
    ...
    try {
      ...
      try {
        ...
        statusEl.textContent = 'Extension needs permission for this website. Enable in Safari \u2192 Settings \u2192 Extensions \u2192 Radarr \u2192 Website Access';
  ...
  clearButton.addEventListener('click', async () => {
    ...
    try {
      ...
      radarrConfigurationLoaded = false;
  ...
  fetchRadarrButton.addEventListener('click', fetchRadarrConfiguration);
  ...
  saveButton.addEventListener('click', async () => {
    ...
    try {
      ...
      initialValues.radarrHost = hostInput.value;
      initialValues.radarrApiKey = apiKeyInput.value;
  ...
  async function fetchRadarrConfiguration() {
    const host = hostInput.value.trim();
    const apiKey = apiKeyInput.value.trim();

    if (!host || !apiKey) {
      showStatusMessage('Please enter both Radarr Host and API Key first.', 'error');
      return;
    }

    fetchRadarrButton.textContent = 'Fetching...';
    fetchRadarrButton.disabled = true;
    ...
    try {
      ...
      showStatusMessage('Error fetching Radarr configuration: ' + error.message, 'error');
      fetchRadarrButton.textContent = 'Fetch Configuration';
      fetchRadarrButton.disabled = false;
    }
  }
